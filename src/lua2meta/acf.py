...
def write_acf(app_info: AppInfo, depot_infos: DepotInfos):
    ...
    acf_contents = {
        "AppState": {
            ...
            "InstalledDepots": {
                depot: installed_depot(depot_info)
                for depot, depot_info in depot_infos.items()
            },
        }
    }
    (args.acf_dir / f"appmanifest_{app_info.appid}.acf").write_text(
        vdf.dumps(acf_contents, pretty=True)
    )
